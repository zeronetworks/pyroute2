...
def _eth_mac_to_keys(mac_bytes, field):
    ...
    if field == 'dst':
        return [
            TcPeditKey(mask=PEDIT_MASK_WRITE_ALL, val=int.from_bytes(mac_bytes[0:4], 'little'), off=0).pack(),
            TcPeditKey(mask=PEDIT_MASK_WRITE_U16_LOW, val=int.from_bytes(mac_bytes[4:6] + b'\x00\x00', 'little'), off=4).pack(),
        ]
    else:
        return [
            TcPeditKey(mask=PEDIT_MASK_WRITE_U16_HIGH, val=int.from_bytes(b'\x00\x00' + mac_bytes[0:2], 'little'), off=4).pack(),
            TcPeditKey(mask=PEDIT_MASK_WRITE_ALL, val=int.from_bytes(mac_bytes[2:6], 'little'), off=8).pack(),
        ]
