...
def _bytes_to_int_le(b):
    """Convert bytes to little-endian integer, Python 2/3 compatible."""
    # Pad to 4 bytes if needed
    b = b + b'\x00' * (4 - len(b))
    return struct.unpack('<I', b)[0]


def _ctypes_to_bytes(ctype_obj):
    """Convert ctypes structure to bytes, Python 2/3 compatible."""
    size = ctypes.sizeof(ctype_obj)
    buf = (ctypes.c_char * size)()
    ctypes.memmove(buf, ctypes.addressof(ctype_obj), size)
